...
用法: python scripts/read_tensorboard_metrics.py runs/ppo_metrics_v3_20251003_195933
"""
...
def find_latest_run():
    """找到最新的训练run"""
    runs = glob.glob('runs/ppo_metrics_v3_*')
    if runs:
...
def main():
    if len(sys.argv) == 1:
        ...
        if not log_dir:
            ...
            print("用法: python scripts/read_tensorboard_metrics.py runs/ppo_metrics_v3_xxx")
            sys.exit(1)
